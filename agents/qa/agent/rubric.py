...
(`console_error`; warnings are excluded), a failed network request
(`failed_request`), or a status value rendered in a casing the page elsewhere
normalises (`status_case_leak`). Each entry has an `id`, a `type`, a `count`,
...
{_fixtures_block()}
   ...
   ONE RENDER BREAK IS REPORTABLE EVEN ON A FIXTURE SURFACE: a status VALUE
   leaking raw into the text -- rendered in the casing it is stored in while
   the page renders that class of value lowercase everywhere else (the same
   class as a raw `user.name` key shown as text). The UI showing its own
   storage is a defect, not a seeded anomaly. Report the leaked value; never
   a figure.
