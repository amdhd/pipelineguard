...
| **D-3** | PR-stable report store (write `pr-<n>/latest` on every PR run) + harness fetches prior report to reconcile | this repo archive step **+ vesselAI workflow flag** | in-repo code **DONE 2026-09-04**; deploy + vesselAI wiring pending | two dispatched QA runs on one PR; second comment shows the re-verify table |
...
- **D-3 still pending (flagged / ops, needed for the two-run verify):** (1) re-package the agent zip + `scripts/apply-dev.sh` from main to deploy the new archiving runtime; (2) give the workflow's CI role `s3:GetObject` on the reports bucket (it is invoke-one-runtime, read-one-secret today); (3) `amdhd/vesselAI` workflow passes `--reports-bucket` and `--report-namespace pr-${{ github.event.pull_request.number }}` to the harness. All three are edits/ops outside this repo's safe-in-repo boundary — flagged, not silently done.
- D-4 → D-5 pending.
