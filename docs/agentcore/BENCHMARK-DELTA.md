...
| **D-3** | PR-stable report store (write `pr-<n>/latest` on every PR run) + harness fetches prior report to reconcile | this repo archive step **+ vesselAI workflow flag** | code + deploy + wiring **DONE 2026-09-04**; live two-run verify pending | two dispatched QA runs on one PR; second comment shows the re-verify table |
...
- **D-3 remaining:** the two-run live verify (merge vesselAI PR #126, then two QA dispatches on one PR — the second comment shows the `🔁 Prior findings re-verified` table). Burns paid runs; not yet requested.
- D-4 → D-5 pending.
